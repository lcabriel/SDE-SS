...
public:
    ...
    virtual float deterministic_part(const valarray<float> &x,float t) const;

    //This function is used to compute the stochastic part of the SDE ( g(x) ) describing the noise accoding to Ito's formula.
    //It has to be override by the different NoisePlugs.
    virtual float stochastic_part(const valarray<float> &x,float t) const;
...
public:
    ...
    float deterministic_part(const valarray<float> &x,float t) const override;

    //Override of the NoisePlug stochastic_part. For the Wiener Process is simply g(x)=1.
    float stochastic_part(const valarray<float> &x,float t) const override;
