...
This class is used to represent a trajectory of a system of SDEs. This is the type of output produced by the *simulateTrajectory* function of a *SDE_SS_System* instance. This a purely object and it is made more for memory optimization, tidiness of the code and unambiguousness. The class contains three elements: the vector of the time instants considered in the simulation, a flat 2D matrix of the values of the variables with time instants->variables order. It can be built both passing another *Traj* element or passing separately the times vector, the flat 2D matrix (in this case some checks are performed) and the number of variables. Once a *Traj* is built there are no way to change the internal values.

- *getTimes()*, *getVars()*, *getLenght()*: these 3 get functions return respectively the three internal variables of the trajectory.
- *flatNotation(size_t TI,unsigned short var)*: you can compute yourself the position in the flat 2D matrix or use this function to obtain the correct index for your (time_instant,variable) couple. This function is entirely constant.
- *vector&lt;float&gt; getInstant(size_t TI)*: given a certain time index, this function will return the features of the system at that index. The output is a vector with the time at the 0th slot and then the values of the variables in the same order in which are saved in the flat 2D matrix.
...
class giving a definition to the virtual function *getData* which will be use to retrive the data from the array when computing the field. This class has already implemented a constructor that accept a *Traj* instance or a time vector and a flat 2D values with a time instant -> variables order (in this case a same dimension check on the two arguments is performed) but you can expand these on your children classes. A DataLinker entity should be theoretically used inside your own custom FieldClass, however for more information about the implementation please look to the *test_code_data.cpp*. Both the array (called *data*) and the TimeOptimizationCounter (called *TOC*; see below) can be accessed by child classes.
